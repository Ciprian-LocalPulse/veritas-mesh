...
- [ ] `core/` Rust crate implementing one narrow rule module end-to-end (progress: all three rule modules now have real Groth16-over-BN254 circuits in `zk-poc/`, AND all three are reachable through `core/`'s own `ProofSystem` trait (`core::proof::groth16_bn254` — `BankingGroth16Backend`, `HealthcareGroth16Backend`, `SupplyChainGroth16Backend`). Still not "end-to-end" in the fuller sense: no orchestration layer yet combines any proof with RFC-0003's commitment and the Ed25519 signature into one attestation-building call, and each rule's `ProvingKey`/`VerifyingKey` are still generated fresh per test rather than loaded from a published ceremony — see `zk-poc/README.md`'s "what's still needed", items 3-4 remain the main open gaps)
- [ ] Real, passing test suite (not a placeholder) (112 tests passing across the repo as of the third circuit's wiring — real assertions, not placeholders, but Phase 2 as a whole isn't done per the item above)
