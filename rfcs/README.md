...
| [0004](0004-verifier-api.md) | Verifier API | Draft — not yet opened for discussion |

The first three are the RFCs named as expected in [Roadmap](../ROADMAP.md)
Phase 0. 0004 fixes the behavioral contract of `VerifierService`
(`proto/veritas/v1/verifier_service.proto`), ahead of any `mesh/`
implementation work (Roadmap Phase 5), so that a first server is written
against a normative spec rather than the other way around. None of the
four has been opened as a pull request or reached the minimum discussion
period defined in [`GOVERNANCE.md`](../GOVERNANCE.md), and none should be
treated as accepted, or as unblocking `core/`, `sdk/`, `mesh/`, or
`spec/formal/` work that depends on RFC acceptance, until that process has
actually run.
...
section. 0004 depends on 0001 only loosely (see 0004's Motivation section
for exactly where the two interact) and not at all on 0002/0003.
